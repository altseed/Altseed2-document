...
このチュートリアルは、Windows ユーザ及び Mac ユーザを想定しています。
...
インストールが終わると、Visual Studio から Microsoft アカウントへのサインインを求められます。
これは必須ではありません。

<img src="install_mac_5.png" height="240px">

キーボードの配置はデフォルトを選択しましょう。

<img src="install_mac_6.png" height="240px">

最後にこの画面が表示されるはずです。
ここまでできたら、晴れて Visual Studio のインストールは完了となります。

<img src="install_mac_7.png" height="240px">
...
次に、.NET Core のバージョンを指定します。
特にバージョンを変更することはないため、このまま「次へ」をクリックしてください。
ここで次に進めないという場合は、.NET Core がインストールされていない可能性があります。
[このページ](https://dotnet.microsoft.com/download/dotnet-core/3.1)から .NET Core をインストールしてください。
...
<img src="project_mac_4.png" height="240px">

試しに、画面左上にある「▶︎」ボタンを押してみましょう。
このボタンを押すと、作成したプログラムのソースコードが実行可能なアプリケーションに自動で変換され、自動で実行されます。
問題なく変換されると、黒い画面が出てきて「Hello World!」と出力されるはずです。

<img src="project_mac_5.png" height="240px">
